...
async def index():
    ...
    <head>
      ...
      <style>
        ...
        .info{position:relative;margin-left:0.5em;cursor:help;color:#333}
        .info:hover{color:#000}
        .info .tooltip{visibility:hidden;opacity:0;position:absolute;bottom:125%;left:50%;transform:translateX(-50%);background:#333;color:#fff;padding:0.2em 0.5em;border-radius:4px;white-space:pre;font-size:0.85em;z-index:10;transition:opacity 0.2s}
        .info:hover .tooltip,.info.active .tooltip{visibility:visible;opacity:1}
      ...
      <form id='cfg'>
        <fieldset>
          ...
          <div class='row' data-type='multiplication'>
            ...
            <span class="info" id="multiplication_info">&#x2139;<span class="tooltip"></span></span>
          </div>
          <div class='row' data-type='addition'>
            ...
            <span class="info" id="addition_info">&#x2139;<span class="tooltip"></span></span>
          </div>
          <div class='row' data-type='subtraction'>
            ...
            <span class="info" id="subtraction_info">&#x2139;<span class="tooltip"></span></span>
          </div>
          <div class='row' data-type='division'>
            ...
            <span class="info" id="division_info">&#x2139;<span class="tooltip"></span></span>
          </div>
          <div class='row' data-type='missingfactor'>
            ...
            <span class="info" id="missingfactor_info">&#x2139;<span class="tooltip"></span></span>
          </div>
          <div class='row' data-type='fractioncompare'>
            ...
            <span class="info" id="fractioncompare_info">&#x2139;<span class="tooltip"></span></span>
          </div>
        </fieldset>
...
(async function(){
  ...
  function exampleFor(type, r1, r2){
    ...
    switch(type){
      ...
      case 'subtraction':
        return `${a2} - ${b1}`;
      case 'division':
        return `${a1*b2} ÷ ${a1} (quotient ${b2})`;
      case 'missingfactor':
        return `${a1} × __ = ${a1*b2}`;
      case 'fractioncompare':
        const n1 = Math.max(1, Math.floor(a1/2));
        const n2 = Math.max(1, Math.floor(b2/2));
        return `${n1}/${a1} ? ${n2}/${b2}`;
      default:
        return '';
    }
  ...
  const DESC = {
    ...
    division: 'divisor (term1) and quotient (term2)',
  ...
  for(const t of types){
    ...
    const info = document.getElementById(`${t}_info`);
    const tooltip = info.querySelector('.tooltip');
    const update = () => {
      try{
        const txt = `${DESC[t]}\n e.g. ${exampleFor(t, t1.value, t2.value)}`;
        info.title = txt; // fallback
        tooltip.textContent = txt;
      }catch{
        info.title = DESC[t];
        tooltip.textContent = DESC[t];
      }
    };
